...
namespace ImdbFilmPage
{
    ...
    public class IMDB
    {
        ...
        public ArrayList MyCast { get; set; }
        ...
        private void parseIMDbPage(string imdbUrl, bool GetExtraInfo)
        {
            ...
            if (!string.IsNullOrEmpty(Id))
            {
                ...
                //parametre aval baraye moshakhas kardane pattern e mast va an (.*?) ghesmati hast k dar parametre dovom 
                //migardad va an ra bargasht midahad...
                //parametre dovom bazei ast k pattern aval dar an jostojo mishavad...
                MyCast = matchAll(@"<td class=""nm""><a.*?href=(.*?)onclick", match(@"<h3>Cast</h3>(.*?)</table>", html));
